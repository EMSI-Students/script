...
if (isset($_GET["send"])) {
    ...
    if (isset($_GET["email"])) {
        $email = filter_var(htmlentities($_GET["email"]), FILTER_SANITIZE_EMAIL);
    }
    ...
    if (isset($_GET["prog_languages"])) {
        foreach ($_GET["prog_languages"] as $langage) {
            $prog_languages[] = htmlentities($langage);
        }
    }

    if (isset($_GET["activities"])) {
        foreach ($_GET["activities"] as $activity) {
